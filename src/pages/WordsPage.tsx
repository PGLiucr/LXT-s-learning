...
const WordsPage = () => {
  ...
  return (
    <div className="space-y-8 relative">
      ...
      {/* Slide-out Sidebar */}
      <div className={`fixed top-0 left-0 h-full w-80 bg-background border-r border-border z-50 transform transition-transform duration-300 ease-in-out ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="py-2">
          {['Dictionary', 'Translate', 'Grammar', 'Thesaurus', 'Pronunciation', 'Cambridge Dictionary +Plus', 'Games'].map((item) => (
            <div key={item} className="px-6 py-4 hover:bg-muted cursor-pointer flex justify-between items-center border-b border-border/50">
              <span className={`font-bold text-slate-900 ${item === 'Cambridge Dictionary +Plus' ? 'text-blue-800' : ''}`}>{item}</span>
              {item === 'Dictionary' && <Plus className="h-4 w-4 text-slate-900" />}
            </div>
          ))}
        </div>

        <div className="mt-auto border-t border-border p-4 space-y-4 bg-slate-50">
          <div className="flex items-center gap-3 px-2 py-2 hover:bg-muted rounded cursor-pointer font-bold text-slate-900">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex gap-3">
          <button onClick={() => setIsLibraryOpen(true)} className="btn-outline flex items-center gap-2">
